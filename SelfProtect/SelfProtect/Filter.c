...
NTSTATUS InitializeFilter(PDRIVER_OBJECT DriverObject)
{
    ...
    if (!NT_SUCCESS(status)) {
        KdPrint(("[SelfProtect] Failed to register filter: 0x%08lX\n", (ULONG)status));
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        gFilterHandle = NULL;
        KdPrint(("[SelfProtect] Failed to start filtering: 0x%08lX\n", (ULONG)status));
        return status;
    }

    KdPrint(("[SelfProtect] File protection initialized\n"));
...
NTSTATUS FilterUnloadCallback(FLT_FILTER_UNLOAD_FLAGS Flags)
{
    ...
    KdPrint(("[SelfProtect] Filter unloaded\n"));
    return STATUS_SUCCESS;
}