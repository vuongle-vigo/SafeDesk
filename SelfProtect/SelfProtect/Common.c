...
PWCHAR CopyUnicodeString(
    ...
    if (Source == NULL || Source->Buffer == NULL || Source->Length == 0 || Source->Length > Source->MaximumLength) {
        DEBUG("[SelfProtect] Invalid Unicode string provided for copying.");
        return NULL;
    }

    // Check IRQL
    if (KeGetCurrentIrql() > DISPATCH_LEVEL) {
        DEBUG("[SelfProtect] CopyUnicodeString called at IRQL > DISPATCH_LEVEL, returning NULL.");
        return NULL; // Don't save to alloc
    }

    if (Source->Length % sizeof(WCHAR) != 0) {
        DEBUG("[SelfProtect] Source Unicode string length is not a multiple of WCHAR size.");
        return NULL;
    }
    ...
    if (dest == NULL) {
        DEBUG("[SelfProtect] Memory allocation failed for CopyUnicodeString.");
        return NULL;
    }
