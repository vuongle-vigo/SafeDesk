...
NTSTATUS AddProtectedFile(_In_ PCUNICODE_STRING FilePath) {
    ...
    if (!protectedFile) {
        KdPrint(("[SelfProtect] Failed to allocate memory for protected file\n"));
    ...
    KdPrint(("[SelfProtect] Added protected file: %wZ\n", &protectedFile->FilePath));
...
VOID CleanupFileProtection(VOID)
{
    ...
    while (!IsListEmpty(&ProtectedFilesList)) {
        ...
        KdPrint(("[SelfProtect] Freed protected file during cleanup\n"));
    }

    // Release the spinlock
    KeReleaseSpinLock(&ProtectedFilesLock, irql);

    KdPrint(("[SelfProtect] File protection cleaned up\n"));
}
...
FLT_POSTOP_CALLBACK_STATUS PostCreateCallback(
    ...
    if (Data->IoStatus.Status == STATUS_ACCESS_DENIED) {
        KdPrint(("[SelfProtect] Access denied for file operation\n"));
    }
...
)
{
    ...
    if (FltObjects->FileObject != NULL) {
        ...
        if (!NT_SUCCESS(ntStatus)) {
            //KdPrint(("[SelfProtect] Failed to get file name information, try with FLT_FILE_NAME_OPENED: 0x%08lX\n", (ULONG)ntStatus));
            ntStatus = FltGetFileNameInformation(Data, FLT_FILE_NAME_OPENED | FLT_FILE_NAME_QUERY_DEFAULT, &nameInfo);
        }
    }

    if (!NT_SUCCESS(ntStatus)) {
        //KdPrint(("[SelfProtect] Failed to get file name information: 0x%08lX\n", (ULONG)ntStatus));
        return NULL;
    }
    ...
    if (nameInfo->Name.Buffer != NULL && nameInfo->Name.Length > 0) {
        ...
        if (filePath == NULL) {
            //KdPrint(("[SelfProtect] Memory allocation failed for file path\n"));
    ...
    if (filePath == NULL) {
        //KdPrint(("[SelfProtect] File path is NULL after processing\n"));
    }
    else {
        //KdPrint(("[SelfProtect] Target file path: %ws\n", filePath));
    }
